...
const ReacodringView = () => {
  ...
  // const run = (value) => {
  //   // Your code to process the transcript (value)
  //   console.log("Processing transcript:", value);
  // };
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/v1/optimize/optimize-text`,
        {
          ...
          body: JSON.stringify({
            user_input: transcript, // Set the transcribed text
            device_tag: 0,          // Assuming 0 is a default tag for the device
            record_time: 0          // Assuming 0 is the placeholder for record time
          }),
        }
      );
     ...
     // Return the saved note
      ...
      setTimeout(() => {
        setCorrectedTranscript(data.data.capture.transcribedText);
  ...
  const saveTranscriptToAPI = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/v1/optimize/optimize-text`,
        {
